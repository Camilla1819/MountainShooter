...
class Menu:
    def __init__(self, window):
        self.window = window
        self.surf = pygame.image.load('./mountain asset/MenuBg.png').convert_alpha()
        self.rect = self.surf.get_rect(left=0, top=0)

    def run(self):
        menu_option = 0  # selecao do menu
        ...
        while True:
            ...
            self.menu_text(35, "Shooter", (255, 255, 0), ((WIN_WIDTH / 2), 120))

            for i in range(len(MENU_OPTION)):
                if i == menu_option:
                    self.menu_text(20, MENU_OPTION[i], (0, 255, 0), ((WIN_WIDTH / 2), 200 + 25 * i))
                else:
